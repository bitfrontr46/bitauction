...
function ChatBox(props) {
    ...
    const handleScroll = (e) => {
        if (e.target.scrollTop === 0) {
            setPage(page + 1)
            setTimeout(socket.current.emit('loading', { ...userInfo, page: page }), 500);
        }
    }

    return (
        <div style={{ textAlign: 'center'}}>
            <Container onScroll={handleScroll} ref={objDiv} className={classes.container}>
                <List className={classes.root}>
                    {showChatList}
                </List>
            </Container>
            <form onKeyDown={onSubmitForm}>
                <Input style={{ width: '93%' }} rowsMax={4} multiline value={message} onChange={onChangeMessage} />
