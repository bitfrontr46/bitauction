...
function EnrollList({ stepIndex, handleEnrollData }) {
  ...
  const onSubmitForm = (e) => {
    ...
    if (tag === "") {
      ...
      handleEnrollData("tags", [...tagList, tag]);
      setTag("");
    }
  };
        ...
        {arr.map((obj) => {
          return (
            <div key={obj.id}>
              <ListItem
                button
                selected={index === obj.id}
                onClick={() => fnc(obj.id, obj.value)}
              >
                <ListItemIcon>
                  <Checkbox checked={index === obj.id} />
                </ListItemIcon>
  ...
  switch (stepIndex) {
    ...
    case 1:
      return (
        <div className={classes.rootList}>
          <h1>
            <QueryBuilderIcon /> 경매 시간을 설정해주세요.
          </h1>
          <p>설정하신 시간 동안 전문가들이 입찰할 것입니다!</p>
          {deadLine === "" ? (
            <></>
          ) : (
            <p>경매 마감 일시: {new Date(deadLine).toLocaleString()}</p>
          )}
    ...
    case 2:
      return (
        <div className={classes.rootList}>
          <h1>
            <CalendarTodayIcon /> 희망 작업 완료일을 선택해주세요.
          </h1>
          {hopeDate}
          <p>전문가와 협의도 가능합니다!</p>
          <List
          ...
          >
            <ListItem
              button
              selected={hopeDateIndex === 0}
              onClick={() => onChangeHopeDate(0, "전문가와 협의")}
            >
              <ListItemIcon>
                <Checkbox checked={hopeDateIndex === 0} />
              </ListItemIcon>
              <ListItemText primary="전문가와 협의" />
            </ListItem>
            <ListItem
              button
              selected={hopeDateIndex === 1}
              onClick={() => onChangeHopeDate(1)}
            >
              <ListItemIcon>
                <Checkbox checked={hopeDateIndex === 1} />
              </ListItemIcon>
    ...
    case 3:
      return (
        <div className={classes.rootList}>
          <h1>
            <LabelIcon /> 아이디어가 있으신가요?
          </h1>
          <p>
            아이디어의 키워드를 태그형식으로 입력해주세요!
            <br />잘 모르겠다면 입력하지 않아도 괜찮습니다.
          </p>
    ...
    case 4:
      return (
        <div className={classes.rootList}>
          <h1>
            <AddCircleOutlineIcon /> 조금 더 자세한 설명을 적어주세요!
          </h1>
          <p>
            전문가들의 이해를 도와줄 설명이 필요해요.
            <br />잘 모르겠다면 전문가와 상담이라고 적어주세요.
          </p>
          <TextField
            ...
            {[
              ...
              },
              {
                ...
                text: tagList.map((obj) => {
                  return (
                    <span key={obj}>
                      <Chip variant="outlined" size="small" label={obj} />
                      &nbsp;
                    </span>
                  );
                }),
